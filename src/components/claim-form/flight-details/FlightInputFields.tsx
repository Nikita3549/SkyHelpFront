...
const FlightInputFields: React.FC<FlightInputFieldsProps> = ({ form }) => {
  ...
  const { flights, isLoading, selectedFlightId, selectFlight } = useFlightData({
    airline,
    departureDate,
    departureAirport,
    arrivalAirport,
  ...
  return (
    <div className="space-y-6">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-600">
                No flights found for the selected date and airline.
              </p>
            </div>
          )}
        </div>
      ...
      {(selectedFlightId === 'not-found' ||
        (!isLoading && showFlightSelection && flights.length === 0)) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField
            control={form.control}
            name="flightNumber"
